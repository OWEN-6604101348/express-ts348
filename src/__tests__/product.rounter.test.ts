...
jest.mock("../controller/product.controller", () => ({
  getAll: (req, res) => {
    return res.status(200).send({
      message: "OK",
      result: [{ id: 6604101348, name: "นาย ปรเมษฐ์ จันทร์ย้อน" }],
    });
  },
  ...
  updateProduct: jest.fn((req, res) => {
    ...
    if (isNaN(id) || typeof name !== "string" || typeof price !== "number") {
      return res.status(400).send({ message: "6604101348 DO THIS FILL WITH HIM SELF" });
    }
    res
      .status(200)
...
describe("Product Router", () => {
  ...
  it("6604101348 DO THIS FILL WITH HIM SELF", async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body).toEqual({ message: "6604101348 DO THIS FILL WITH HIM SELF" });
  });
});